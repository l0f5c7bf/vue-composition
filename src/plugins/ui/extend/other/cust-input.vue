...
export default {
  ...
  methods: {
    remoteMethod(query) {
      if (query !== '') {
        this.$api('common.findCustByCustName', { custName: query }).then(res => {
  ...
  watch: {
    value(newValue, oldValue) {
      if (newValue && newValue !== oldValue && !this.options.some(v => v.id === newValue)) {
        this.$api('common.findCustNameByCustId', { custId: this.value }).then(response => {
          this.options = [{
